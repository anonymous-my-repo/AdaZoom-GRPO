...
def _inner_api_call(answer_text, ground_truth, question_text, client, model_name):
    ...
    response = prompt_client(client, model_name, full_prompt, temperature=0.3)
    # print(response)
...
def compute_score(predict_str: str, ground_truth: str, extra_info=None) -> float:
    ...
    mass_dict = {
        "acc": 0.8 * acc_reward,
        "format": 0.2 * format_reward,
        "tool": 1.2 * tool_reward,
        "cof": 0.0,
        # "iou": 0.2 * iou_reward,
        "process": 0.0,
    }
...
def compute_common_reasoning(predict_str: str, ground_truth: str, extra_info=None) -> float:
    ...
    if not answer_text:
        ...
        for ix in range(8):
            ...
            # )
            # response = chat_response.choices[0].message.content.strip()
            response = prompt_client(client, model_name, full_prompt, temperature=0.5, seed=random.randint(0, 1000000))
...
def generative_verify(query, ground_truth, model_answer):
    ...
    for it in range(8):
        try:
            ...
            # )
            # response = chat_response.choices[0].message.content.strip()
            response = prompt_client(client, model_name, full_prompt, temperature=0.0, seed=random.randint(0, 1000000))
            break
