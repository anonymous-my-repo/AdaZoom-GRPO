...
def compute_agent_metrics(batch: DataProto):
    ...
    for term in target_reward_terms:
        if term in non_tensor:
            val = non_tensor[term]
            if len(val) > 0:
                metrics[f"reward/{term}"] = np.mean(val)
            else:
                metrics[f"reward/{term}"] = 0.0
        else:
            metrics[f"reward/{term}"] = 0.0
    if "format" in non_tensor:
        val = non_tensor["format"]
        metrics["reward/format_correct_cnt"] = sum(val == 0)
        metrics["reward/format_error_cnt"] = sum(val == -1)
    else:
        metrics["reward/format_correct_cnt"] = 0
        metrics["reward/format_error_cnt"] = 0
    return metrics
